export const generateScheduleCode = () => {
    const generateSchedule = document.querySelector("#generateSchedule");
    
    generateSchedule.addEventListener("click", () => {
        const selectTag = document.querySelectorAll(".tag");
        
        let codeCourses = [];
        if(selectTag){
            selectTag.forEach((item) => {
                let tagCode = item.innerHTML.replace('<span class="remove-tag">✖</span>', '');
                if (!codeCourses.includes(tagCode)){
                    codeCourses.push(tagCode);
                }
            })
        }

        viewCourseSelect(codeCourses);
        ...
        for(let j = 0; j < horarioCompleto[0].length; j++){
            if(horarioCompleto[i][j] != ""){
                if(horarioCompleto[i][j].includes("-")){
                    let curso1indice = parseInt(horarioCompleto[i][j].split("-")[0][0]);
                    let curso2indice = parseInt(horarioCompleto[i][j].split("-")[1][0]);
                    let curso1completo = encontrarCursoXIndice(arregloCursos[curso1indice-1].split("/")[0], arregloCursos[curso1indice-1].split("/")[1], datatable)
                    let curso2completo = encontrarCursoXIndice(arregloCursos[curso2indice-1].split("/")[0], arregloCursos[curso2indice-1].split("/")[1], datatable)
...
function generateSchedule(resultsCombinaciones) {
    fetch("../JSON/horarioUltimo.json")
    .then(response => response.json())
    .then(data => {
        let dataHorario = data;

        for (let i = 0; i < resultsCombinaciones.length; i++) {
            let linea1 = crearLineaCursos(resultsCombinaciones[i],dataHorario);
            ...
            generateScheduleTable(resultsCombinaciones[i], horarioCreado, i);
        }

    })
}

function generateScheduleTable(lineaDeEntrada, horarioCreado, w) {
        ...
        for (let j = 0; j < horarioCreado[1][i].length; j++ ) {
            if(horarioCreado[1][i][j] !== '') {
                ...
                for (let k = 0; k < horarioCreado[1][i][j].split("-").length; k++) {
                    let codeCourse = lineaDeEntrada[horarioCreado[1][i][j].split("-")[k].split("")[0] - 1];
                    let type = horarioCreado[1][i][j].split("-")[k].split("")[1];
    ...
    try {
        ...
        arraySelect.forEach(course => {
            ...
            const idf = document.querySelector(`#${dayId}_${w}`);
...
function detectarSecciones(dataTable, CodigoCurso){
    let respuesta = Object.keys(dataTable[CodigoCurso].Secciones);
    return respuesta;
}

function encontrarCursoXCodigoTwo(codigo, dataTable){
    let codigoCursoEncontrado = "";
    let respuesta = "";
    for(const curso in dataTable){
        if(dataTable[curso].Codigo == codigo){
...
function obtenerCombinaciones(lineaCurso, lineaSecciones){
    console.log("lineaCurso -->",lineaCurso);
    console.log("lineaSecciones -->", lineaSecciones);
    ...
    let numPush = 0;
    for(let i = 0; i<8; i++){
        if(i <lineaSecciones.length){
            lineaFinal.push(lineaSecciones[i]);
        }
        else{
            lineaFinal.push(['']);
            numPush++;
    ...
    let result = cartesian(lineaFinal[0],lineaFinal[1],lineaFinal[2],lineaFinal[3],lineaFinal[4],lineaFinal[5],lineaFinal[6],lineaFinal[7])
    ...
    for(let i = 0; i < result.length; i++){
        for(let j = 0; j < lineaCurso.length; j++){
            result[i][j] = lineaCurso[j]+result[i][j]
        }
    }
    return result
}

function crearHorario(lineaCurso, lineaSecciones){
    let resultsCombinaciones = []
    let combinaciones = obtenerCombinaciones(lineaCurso, lineaSecciones)
    for(let com of combinaciones){
        resultsCombinaciones.push(com);
    }

    console.log("Combinaciones -->", resultsCombinaciones);

    paintCellSchedule(resultsCombinaciones);
...
const viewCourseSelect = (lineaCurso) => {
    ...
    .then(dataTable => {
        ...
        let lineaIndiceCursos = [];
        let lineaSecciones = [];

        for(let curso in lineaCurso) {
            lineaIndiceCursos.push(encontrarCursoXCodigoTwo(lineaCurso[curso], dataTable));
            lineaSecciones.push(detectarSecciones(dataTable, lineaIndiceCursos[curso]));
            // console.log(dataTable[lineaIndiceCursos[curso]].Secciones.lineaSecciones[0]);
        }

        crearHorario(lineaCurso, lineaSecciones);
    });
}

function paintCellSchedule(resultsCombinaciones) {

    const scheduleGroup = document.querySelector("#generateCellSchedule");

    if (resultsCombinaciones.length !== 1) {
        for (let i = 0; i < resultsCombinaciones.length; i++) {
            scheduleGroup.insertAdjacentHTML("beforeend", `
                <div>${resultsCombinaciones[i]}</div>
                <section id="cell_section_${i}" >
                    <div  class="calendar_main" style="
                        position: relative;
                        width: 100%;
                        visibility: visible;
                        cursor: move;
                            ...
                            <tr>
                                ...
                                <div style="position: relative">
                                    ...
                                    <table cellpadding="0" cellspacing="0" border="0" style="
                                        ...
                                        ">
                                    <tbody>
                                        <tr class="paint_cell paint_cell_${i}"></tr>
                                    </tbody>
                                    </table>
                                </div>
                                </td>
            ...
            `);
            generateCellSchedule(i);
        }
    }
    generateSchedule(resultsCombinaciones);
}

function generateCellSchedule(k) {
    const hours = document.querySelector(`#hours_${k}`);
    const paintcellTwo = document.querySelector(`.paint_cell_${k}`);
    const bodycell = document.querySelector(`#body_cell_${k}`);
    ...
    for (let i = 0; i < 8; i ++) {
        paintcellTwo.insertAdjacentHTML("beforeend", `<td style="padding: 0px; border: 0px none; height: 0px; overflow: visible; text-align: left;">
            <div class="alldays" id="${days[i]}_${k}" style="margin-right: 5px; position: relative; height: 1px; margin-top: -1px;"></div>
    ...
    for (let i = 7; i < 41; i ++) {
        ...
        let tr = `<tr class="cell_hour_${k} hour_${stringCount}"></tr>`;
        bodycell.insertAdjacentHTML('beforeend', tr);
    }

    const trcell = document.querySelectorAll(`.cell_hour_${k}`);
    ...
    for (let i = 0; i < trcell.length; i++) {
        ...
        days.forEach(day => {
            cellhourcourse = cellhourcourse + `<td id="${day}_${trcell[i].getAttribute('class').replace('cell_hour hour_', '')}_${k}" style="padding: 0px; border: 0px none; vertical-align: top; height: 30px; overflow: hidden;">
...
const buttonFooterClose = document.querySelector("#button__footer__close");
let generateCellScheduleId = document.querySelector("#generateCellSchedule");

buttonClose.addEventListener('click', event => {
    generateCellScheduleId.innerHTML = "";
});

buttonFooterClose.addEventListener('click', event => {
    generateCellScheduleId.innerHTML = "";
});